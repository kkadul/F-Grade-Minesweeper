...
class FGradeFinder:
    def __init__(self, root):
        ...
        try:
            ...
            print("알림: 'background.mp3' 파일을 찾을 수 없어 배경음악 없이 시작합니다.")
    ...
    def setup_ui(self):
        ...
        self.volume_slider = Scale(
            self.bottom_control_frame, from_=0, to=100, orient=tk.HORIZONTAL, command=self.set_volume, length=150, showvalue=0,
            bg="#c0c0c0", troughcolor="#e0e0e0", sliderrelief=tk.FLAT, highlightthickness=0, activebackground="#a0a0a0"
        )
    ...
    def restart_game(self):
        ...
        if hasattr(self, 'board_frame'):
            ...
            self.board_frame = tk.Frame(self.root); self.board_frame.pack(padx=10, pady=5)
    ...
    def place_fgrades(self, first_click_pos):
        ...
        safe_area = {(first_click_pos[0]+dx, first_click_pos[1]+dy) for dx in range(-1, 2) for dy in range(-1, 2)}
        while len(self.fgrade_positions) < self.fgrades:
            pos = (random.randint(0, self.rows-1), random.randint(0, self.cols-1))
            if pos not in safe_area: self.fgrade_positions.add(pos)
